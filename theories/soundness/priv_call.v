...
Section proofs.
  ...
  Proof.
    ...
    assert (hwfthis_ : wf_ty (ClassT false t σ)).
    { rewrite -ht0.
      by apply wflty.
    }
    ...
    assert (hrecv: expr_has_ty Δ Γ (length Σ) kd ThisE (ClassT false t σ)).
    { rewrite -ht0.
      by apply ThisTy.
    }
    ...
    assert (hokt: ok_ty Δ (ClassT true t σ)).
    { destruct Γ as [[] ?].
    ...
    assert (hok0_: ok_ty Δ (ClassT true C (subst_ty σ <$> σt_o))).
