...
Local Notation "s <: t" := (@subtype Δ s t) (at level 70, no associativity).
...
Proof.
  ...
  - inv hc.
    ...
    { apply (@has_fields_inherits_lookup Δ) with t0 t => //.
      by apply wfΔ.
    }
  ...
  - iIntros "[Hh #Hle]".
    inv hc; simpl in *.
    assert (wfbody: ∃B, @wf_mdef_ty Δ B mdef0 ∧ inherits t0 B).
    ...
    assert (hincl: @mdef_incl Δ mdef0 mdef).
    {
