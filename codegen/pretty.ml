...
let rec lang_ty_pretty = function
  ...
  | NullT -> "NullT"
  | NonNullT -> "NonNullT"
  | UnionT (s, t) -> sprintf "(UnionT (%s) (%s))"
    (lang_ty_pretty s) (lang_ty_pretty t)
  | InterT (s, t) -> sprintf "(InterT (%s) (%s))"
    (lang_ty_pretty s) (lang_ty_pretty t)
...
let rec expr_pretty = function
  ...
  | NullE -> "NullE"
  | BinOpE {op; lhs; rhs } ->
    sprintf "(BinOpE %s (%s) (%s))"
      (show_binop op) (expr_pretty lhs) (expr_pretty rhs)
  | UniOpE { op; e } ->
    sprintf "(UniOpE %s (%s))"
      (show_uniop op) (expr_pretty e)
  | VarE s -> sprintf "(VarE %s)" s
  | ThisE -> "ThisE"
  | UpcastE {e; ty} ->
    sprintf "(Upcast (%s) (%s))"
      (expr_pretty e) (lang_ty_pretty ty)
...
let rec cmd_pretty = function
  ...
  | LetC {lhs; e} ->
    sprintf "(LetC \"%s\" (%s))" lhs (expr_pretty e)
  | IfC {cond; thn; els} ->
    sprintf "(IfC (%s) (%s) (%s))" (expr_pretty cond)
      (cmd_pretty thn) (cmd_pretty els)
  | GetC {lhs; recv; name} ->
...
let methodDef_pretty cname { name; args; return_type; body; return } =
  ...
  let l = [ "{|";
            ...
            sprintf "  methodvisibility := Public;"; (* TODO *)
            "|}" ] in
  let s = String.concat "\n" l in
  sprintf "Definition %s_%s := %s."
    cname name s

let classDef_pretty { name; generics = _; constraints; super; fields;
                      methods
                    } =
  let cs = List.flatten @@ List.map (fun (var, l, r) ->
      let l = lang_ty_pretty l in
      let r = lang_ty_pretty r in
      match var with
      | Eq -> [(l, r); (r, l)]
      | As -> [(l, r)]
      | Super -> [(r, l)]) constraints in
  let cs = List.map (fun (l, r) ->
      sprintf "(%s, %s)" l r) cs in
  ...
  in
  let fields =
    List.map (fun (vis, ty, name) ->
        sprintf "\"%s\" := (%s, %s)" name (show_visibility vis)
          (lang_ty_pretty ty)) fields in
